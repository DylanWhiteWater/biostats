...
may_plot <- ggplot(may_data, aes(x = elev, y= t_average)) + geom_point() + geom_smooth(method = lm) + xlab("Elevation") +ylab("Temperature")
may_plot
dec_plot <-  ggplot(dec_data, aes(x = elev, y= t_average)) + geom_point() +geom_smooth(method = lm) + xlab("Elevation") +ylab("Temperature")
dec_plot
...
lapserate.function<- function(l){
  for(i in 1:length(subset_clim_4$month))#192 long?
    ...
    print(c(subset_clim_4$month,lr_lm$coef[2],summary(lr_lm)$r.squared))
    print(ggplot(subset_clim_4, aes(x = subset_clim_4$month, y = lr_lm$coef[2])) + geom_point() + geom_line())
